...
// A IA responde sempre "Pasta", com confiança alta.
...
globalThis.fetch = async (url, opts) => {
  ...
  return { ok: true, status: 200, json: async () => ({
    content: [{ type: "text", text: JSON.stringify({ etapa: "Pasta", confianca: "alta",
      porque: "O cliente enviou os documentos.", trecho: "segue aí" }) }],
    usage: { input_tokens: 900, output_tokens: 80 } }) };
};

const { default: db } = await import("../src/db.js");
const { randomUUID } = await import("crypto");
const { previaTemperatura, limparTemperatura, previaEtapaIA, rodarEtapaIA } = await import("../src/services/lote.js");
